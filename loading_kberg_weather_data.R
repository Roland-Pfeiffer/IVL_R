...
PATH_TO_KBERG_FILES <- "/media/findux/DATA/Documents/IVL/Data/Kberg_weather_data/originals/"
SAVE_PATH <- "/media/findux/DATA/Documents/IVL/Data/Kberg_weather_data/"
...
for (fname in fnames_kberg){
  fpath <- paste(PATH_TO_KBERG_FILES, fname, sep = "/")
  tmp <- read.csv(fpath)
  tmp <- tmp[,2:8]
...
colnames(data_3m) <- c("datetime", "sal_PSU", "temp")
data_3m$depth_m <- 3

data_7m <- subset(data_kberg, select = c(datetime, sal_7, temp_7))
colnames(data_7m) <- c("datetime", "sal_PSU", "temp")
data_7m$depth_m <- 7

data_25m <- subset(data_kberg, select = c(datetime, sal_25, temp_25))
colnames(data_25m) <- c("datetime", "sal_PSU", "temp")
data_25m$depth_m <- 25
...
temp_jetty <- ggplot(data = data_kberg[data_kberg$depth_m == "3", ],
...
sal_jetty <- ggplot(data = data_kberg[data_kberg$depth_m == "3", ],
                    mapping = aes(x = datetime, y=sal_PSU)) +
  geom_line() + 
  scale_x_datetime(limits = xlims, date_breaks = "1 month", date_labels = "%b-'%y") +
  ggtitle("Jetty") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
temp_7 <- ggplot(data = data_kberg[data_kberg$depth_m == "7", ],
                 mapping = aes(x = datetime, y=temp)) +
  geom_line() + 
  scale_x_datetime(limits = xlims, date_breaks = "1 month", date_labels = "%b-'%y") +
  ggtitle("07 m") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
sal_7 <- ggplot(data = data_kberg[data_kberg$depth_m == "7", ],
                mapping = aes(x = datetime, y=sal_PSU)) +
  geom_line() + 
  scale_x_datetime(limits = xlims, date_breaks = "1 month", date_labels = "%b-'%y") +
  ggtitle("07 m") +
...
temp_25 <- ggplot(data = data_kberg[data_kberg$depth_m == "25", ],
                  mapping = aes(x = datetime, y=temp)) +
  geom_line() + 
  scale_x_datetime(limits = xlims, date_breaks = "1 month", date_labels = "%b-'%y") +
  ggtitle("25 m") +
...
sal_25 <- ggplot(data = data_kberg[data_kberg$depth_m == "25", ],
                 mapping = aes(x = datetime, y=sal_PSU)) +
  geom_line() + 
  scale_x_datetime(limits = xlims, date_breaks = "1 month", date_labels = "%b-'%y") +
  ggtitle("25 m") +
...
write.csv(data_kberg, paste(SAVE_PATH, "kberg_data_merged_", time_now, ".csv", sep = ""),
          row.names = FALSE)
