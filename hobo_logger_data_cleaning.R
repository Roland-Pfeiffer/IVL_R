...
adjust_datetime <- function(dframe){
  ...
  dframe$datetime <- as.POSIXct(dframe$datetime, format="%m/%d/%y %I:%M:%S %p")
...
# TODO: make this interactive with user input
...
# fname <- "/media/findux/DATA/Documents/IVL/Data/HOBO logger data 2020/200528_nedsänk_ytan.csv"
# data <- read.csv(fname, skip = 1, check.names = FALSE)
# data <- add_gmt_offset(data)
